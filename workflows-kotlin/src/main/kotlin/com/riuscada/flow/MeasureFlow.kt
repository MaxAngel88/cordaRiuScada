...
object MeasureFlow {
    ...
    class Updater(val measureLinearId: String,
        ...
        override fun call(): MeasureState {
            ...
            if (oldMeasureStateList.size > 1 || oldMeasureStateList.isEmpty()) throw FlowException(
                "No measure state with UUID: " + UUID.fromString(
                    measureLinearId
                ) + " found."
            )
            ...
            val txCommand =
                Command(MeasureContract.Commands.Update(), newMeasureState.participants.map { it.owningKey })
            val txBuilder = TransactionBuilder(notary)
    ...
    */
    ...
    @InitiatingFlow
    @StartableByRPC
    class Updater(val hostname: String,
                  val macAddress: String,
                  val time: Instant,
                  val xmlData: String,
                  val uuid: String) : FlowLogic<MeasureState>() {
        ...
        override fun call(): MeasureState {
            ...
            if (oldMeasureStateList.isEmpty()) throw FlowException("No measure state to consume for macAddress: $macAddress found.")
